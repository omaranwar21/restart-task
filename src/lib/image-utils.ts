...
export function getValidImageUrl(
  ...
  // Default placeholder
  const placeholder = `https://picsum.photos/${width}/${height}?random=${Math.floor(Math.random() * 1000)}`
  ...
  try {
    ...
    const isImageService = imageServiceDomains.some(domain => url.hostname.includes(domain))
...
export function getValidImageUrlWithCategory(
  ...
  if (validUrl !== imageUrl && category) {
    ...
    const keyword = categoryKeywords[category.toLowerCase() as keyof typeof categoryKeywords] || 'product'
    return `https://loremflickr.com/${width}/${height}/${keyword}?random=${Math.floor(Math.random() * 1000)}`
  }
